...
function load() {
    return new Promise(function (resolve, reject) {
        try {
            data = { devices: {}, hmi: { views: [] }, texts: [], alarms: [],timestamp: Date.now().toLocaleString() };
            // load general data
            prjstorage.getSection(prjstorage.TableType.GENERAL).then(grows => {
                logger.info('project.load: GENERAL section loaded');
                if (grows && grows.length > 0) {
                    for (var ig = 0; ig < grows.length; ig++) {
                        if (grows[ig].name === ProjectDataCmdType.HmiLayout || grows[ig].name === ProjectDataCmdType.MobileLayout || grows[ig].name === ProjectDataCmdType.ViewFolders) {
                ...
                prjstorage.getSection(prjstorage.TableType.VIEWS).then(vrows => {
                    logger.info('project.load: VIEWS section loaded');
                    ...
                    prjstorage.getSection(prjstorage.TableType.DEVICES).then(drows => {
                        logger.info('project.load: DEVICES section loaded');
                        if (drows && drows.length > 0) {
                            for (var id = 0; id < drows.length; id++) {
                                var device = JSON.parse(drows[id].value);
                                // Load all devices including server into data.devices
                                data.devices[drows[id].name] = device;
                                // If this is the server device (id='0'), also set it as data.server for backward compatibility
                                if (device.id === '0') {
                                    data.server = device;
                                }
                            }
                        ...
                        async.series([
                            ...
                            function (callback) {
                                getTexts().then(texts => {
                                    logger.info('project.load: TEXTS loaded');
                                    data.texts = texts;
                                    callback();
                                }).catch(function (err) {
                            ...
                            function (callback) {
                                getAlarms().then(alarms => {
                                    logger.info('project.load: ALARMS loaded');
                                    data.alarms = alarms;
                                    callback();
                                }).catch(function (err) {
                            ...
                            function (callback) {
                                getNotifications().then(notifications => {
                                    logger.info('project.load: NOTIFICATIONS loaded');
                            ...
                            function (callback) {
                                getScripts().then(scripts => {
                                    logger.info('project.load: SCRIPTS loaded');
                            ...
                            function (callback) {
                                getReports().then(reports => {
                                    logger.info('project.load: REPORTS loaded');
                            ...
                            function (callback) {
                                getMapsLocations().then(locations => {
                                    logger.info('project.load: LOCATIONS loaded');
                        ...
                        async function (err) {
                            if (err) {
                                ...
                                logger.info('project.load: calling _mergeDefaultConfig');
                                await _mergeDefaultConfig();
                                logger.info('project.load: completed successfully');
                                resolve();
                            }
                        });
                    }).catch(function (err) {
            ...
            logger.error(`project.load: unexpected error ${err}`);
            reject(err);
        }
    });
}
...
function setProjectData(cmd, value) {
    ...
    try {
        if (cmd === ProjectDataCmdType.SetView) {
        ...
        } else if (cmd === ProjectDataCmdType.SetDevice || cmd === ProjectDataCmdType.SetDeviceTags) {
            ...
            section.name = cmd;
            setGraphs(value);
        } else if (cmd === ProjectDataCmdType.ViewFolders) {
            section.table = prjstorage.TableType.GENERAL;
            section.name = cmd;
            setViewFolders(value);
            ...
            return Promise.reject('prjstorage.failed-to-setdata: Command not found!');
        }
    } catch (err) {
    ...
    if (toremove) {
        return prjstorage.deleteSection(section).catch(function (err) {
            logger.error(`prjstorage.deletedata failed! '${section.table}'`);
            throw err;
        });
    } else {
        const newSection = getTimeSection();
        let sections = [section, newSection];
        return prjstorage.setSections(sections).then(function () {
            data.timestamp = newSection.value;
        }).catch(function (err) {
            logger.error(`prjstorage.setdata failed! '${section.table}'`);
            throw err;
        });
...
/**
 * Set ViewFolders to hmi
 * @param {*} viewFolders
 */
function setViewFolders(viewFolders) {
    data.hmi.viewFolders = viewFolders;
}
...
function getProject(userId, userPermission) {
    return new Promise(function (resolve, reject) {
        const pdata = _filterProjectPermission(userId, userPermission);
        resolve(pdata);
    });
}
            ...
            prjstorage.clearAll().then(result => {
                ...
                Object.keys(prjcontent).forEach((key) => {
                    if (key === 'devices') {
                        ...
                        if (hmi && typeof hmi === 'object' && Object.keys(hmi).length > 0) {
                            Object.keys(hmi).forEach((hk) => {
                                if (hk === 'views') {
                                    // views
                                    if (hmi[hk] && Array.isArray(hmi[hk]) && hmi[hk].length > 0) {
                        ...
                        // server - Skip saving separately as it's already in devices['0']
                        // The server device should be saved with id='0', not name='server'
                        // This avoids duplicate entries in the database
                        // Do nothing, just skip this key
                    } else if (key === 'texts') {
                        // texts
...
/**
 * Return device folders dictionary
 */
function getDeviceFolders() {
    return data.deviceFolders || {};
}
    ...
    return new Promise(function (resolve, reject) {
        if (query.query === 'security') {
            if (!query.value) {
             ...
             const newSection = getTimeSection();
            let scs = [newSection,{ table: prjstorage.TableType.DEVICESSECURITY, name: query.name, value: query.value }];
            prjstorage.setSections(scs).then(() => {
                resolve();
                data.timestamp = newSection.value;
            }).catch(function (err) {
...
function _filterProjectPermission(userId, userPermission) {
    var result = JSON.parse(JSON.stringify(data));// = { devices: {}, hmi: { views: [] } };
    // If project data hasn't been loaded yet (hmi not initialized), return early
    if (!result.hmi) {
        return result;
    }
    //TODO 增加判断PC端和移动端
    const projectPermission = runtime.checkPermission(userPermission, false);
    // Guest token is auto-generated for unauthenticated users; userId === 'guest' is reliable
    const isNotLoggedIn = (userId === 'guest') && settings.secureEnabled;
    if (!projectPermission.show || !projectPermission.enabled) {   // is admin or secure disabled
        ...
        // Keep server in sync with devices['0'] for backward compatibility
        if (result.devices && result.devices['0']) {
            result.server = result.devices['0'];
        } else {
        ...
        if (Array.isArray(result.hmi.views) && result.hmi.views.length > 0) {
            for (var i = 0; i < result.hmi.views.length; i++) {
                var view = result.hmi.views[i];
                if (result.hmi.views[i].items && typeof result.hmi.views[i].items === 'object') {
                    Object.values(result.hmi.views[i].items).forEach((item) => {
                        if (item.property) {
                            ...
                            if (!itemPermission.show) {
                                ...
                                console.log('[DEBUG-PERM] item', item.id, 'enabled=false, isNotLoggedIn:', isNotLoggedIn, 'events:', item.property.events ? item.property.events.length : 0);
                                if (isNotLoggedIn) {
                                    // Not logged in: keep events, mark as requiring login
                                    item.property._requiresLogin = true;
                                    // Mark foreignObject with data-requires-login (don't disable inner controls)
                                    ...
                                    // Logged in but insufficient permission: original behavior
...
function _mergeDefaultConfig() {
    return new Promise(async function (resolve, reject) {
        try {
            logger.info('_mergeDefaultConfig: starting');
            ...
            logger.info('_mergeDefaultConfig: completed');
            resolve();
        } catch (err) {
...
function getTimeStamp(){
   return data.timestamp || new Date().toLocaleString();
}
...
module.exports = {
    ...
    getDeviceFolders: getDeviceFolders,
    getDevice: getDevice,
    getAlarms: getAlarms,
