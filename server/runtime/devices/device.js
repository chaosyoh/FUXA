...
function Device(data, runtime) {
    ...
    // Degrade (backoff) reconnection mechanism
    var degradeEnabled = (data.degradeEnabled !== undefined) ? !!data.degradeEnabled : true;   // Whether to use degrade mode
    var degradeRetryCount = (data.degradeRetryCount !== undefined) ? parseInt(data.degradeRetryCount) || 2 : 2; // Retry count before entering degrade mode
    var degradePeriod = (data.degradePeriod !== undefined) ? parseInt(data.degradePeriod) || 60 : 60;           // Degrade period in seconds
    var degraded = false;                   // Whether currently in degrade mode
    var degradeLastAttempt = 0;             // Timestamp of last reconnect attempt in degrade mode
    var degradeRetryAttempts = 0;           // Current retry attempts counter
    ...
    this.checkStatus = function () {
        if (status === DeviceStatusEnum.INIT && currentCmd === DeviceCmdEnum.START) {
            ...
            // Degrade mode: skip reconnect attempt if not enough time has elapsed
            var skipConnect = false;
            if (degradeEnabled && degraded) {
                var elapsed = Date.now() - degradeLastAttempt;
                if (elapsed < degradePeriod * 1000) {
                    skipConnect = true; // Still within degrade period
                } else {
            ...
            if (!skipConnect) {
                this.connect().then(() => {
                    tryToConnect = 0;
                    // Reset degrade state on successful connection
                    degraded = false;
                    degradeRetryAttempts = 0;
                    degradeLastAttempt = 0;
                    status = DeviceStatusEnum.IDLE;
                    self.restoreValues();
                }).catch(function (err) {
                    if (degradeEnabled) {
                        degradeRetryAttempts++;
                        logger.error(`'${property.name}' connect error! ${err} (attempt ${degradeRetryAttempts}/${degradeRetryCount})`);
                        if (degraded) {
                            // Already in degrade mode, just log and wait for next degrade cycle
                            logger.warn(`'${property.name}' still in degrade mode, next retry in ${degradePeriod}s`);
            ...
            status = DeviceStatusEnum.INIT;
            // Reset degrade state when connection drops, allow normal retries first
            degraded = false;
            degradeRetryAttempts = 0;
            degradeLastAttempt = 0;
            if (devicePolling) {
    ...
    this.setValue = async function (id, value, fnc) {
        // Check access permission: read-only tags cannot be written
        if (data.tags[id] && data.tags[id].access === 'ro') {
            logger.warn(`'${data.name}' setValue(${id}) rejected: tag is read-only`);
            return false;
        }
