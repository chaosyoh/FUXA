...
function load() {
    ...
    if (!tempdevices || typeof tempdevices !== 'object') {
        runtime.logger.error('devices.load: getDevices() returned null/undefined!');
        tempdevices = {};
    }
    activeDevices = {};
    ...
    for (var id in tempdevices) {
        if (tempdevices[id] && tempdevices[id].enabled) {
            try {
                ...
                runtime.logger.error(`devices.load: failed to load device '${tempdevices[id].name || id}': ${err}`);
                if (err && err.stack) runtime.logger.error(`devices.load stack: ${err.stack}`);
            }
        }
    }
