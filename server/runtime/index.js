...
const alarmstorage = require('./alarms/alarmstorage');
...
function init(_io, _api, _settings, _log, eventsMain) {
    ...
    // Serial initialization of all storage modules to avoid DB connection
    // race conditions (especially important for MSSQL shared pool).
    (async () => {
        try {
            // 1. DAQ storage (includes currentstorage)
            await daqstorage.init(settings, logger, runtime);
            logger.info('runtime init daqstorage successful!', true);

            // 2. Scheduler storage + service
            await schedulerStorage.init(settings, logger, runtime);
            await schedulerService.init(settings, logger, runtime);
            logger.info('runtime init scheduler services successful!', true);

            // 3. Plugins
            await plugins.init(settings, logger);
            logger.info('runtime init plugins successful!', true);
            events.emit('init-plugins-ok');

            // 4. API Keys
            await apiKeys.init(settings, logger);
            logger.info('runtime init apiKeys successful!', true);

            // 5. Users
            await users.init(settings, logger);
            logger.info('runtime init users successful!', true);
            events.emit('init-users-ok');

            // 6. Project
            await project.init(settings, logger, runtime);
            logger.info('runtime init project successful!', true);
            events.emit('init-project-ok');

            // 7. Alarm storage (must init before AlarmsManager state machine runs)
            await alarmstorage.init(settings, logger);
            logger.info('runtime init alarmstorage successful!', true);
        } catch (err) {
            logger.error('runtime.failed-to-init storage modules: ' + err);
            if (err && err.stack) logger.error('runtime.failed-to-init stack: ' + err.stack);
        }
    })();
    ...
    io.on('connection', async (socket) => {
        ...
        // editor requests to notify all clients about project update
        socket.on(Events.IoEventTypes.PROJECT_NOTIFY_UPDATE, () => {
            try {
                const timestamp = project.getTimeStamp();
                logger.info(`[PROJECT_NOTIFY_UPDATE] Broadcasting project-updated to all clients, timestamp: ${timestamp}`);
                io.emit(Events.IoEventTypes.PROJECT_UPDATED, { timestamp });
            } catch (err) {
                logger.error(`${Events.IoEventTypes.PROJECT_NOTIFY_UPDATE}: ${err}`);
            }
        });
        ...
        socket.on(Events.IoEventTypes.DEVICE_RESTART, (message) => {
            try {
                if (!isSocketWriteAuthorized(socket)) {
                    logger.warn(`${Events.IoEventTypes.DEVICE_RESTART}: unauthorized restart attempt from ${socket.userId || 'guest'}`);
                    return;
                }
                if (message && message.device) {
                    var device = project.getDevice(message.device);
                    if (device) {
                        devices.updateDevice(device);
                        logger.info(`${Events.IoEventTypes.DEVICE_RESTART}: device '${device.name}' restarted`, true);
                    }
                }
            } catch (err) {
                logger.error(`${Events.IoEventTypes.DEVICE_RESTART}: ${err}`);
            }
        });
...
function start() {
    return new Promise(function (resolve, reject) {
        // load project
        logger.info('runtime.start: loading project...');
        project.load().then(result => {
            logger.info('runtime.start: project loaded successfully');
            try {
                ...
                devices.start().then(function () {
                    ...
                    if (err && err.stack) logger.error('runtime.failed-to-start-devices stack: ' + err.stack);
                    reject();
                });
            } catch (err) {
                logger.error('runtime.failed-to-start-devices-sync: ' + err);
                if (err && err.stack) logger.error('runtime.failed-to-start-devices-sync stack: ' + err.stack);
                reject();
            }
            ...
            alarmsMgr.start().then(function () {
                ...
                if (err && err.stack) logger.error('runtime.failed-to-start-alarms stack: ' + err.stack);
                reject();
            });
            ...
            notificatorMgr.start().then(function () {
                ...
                if (err && err.stack) logger.error('runtime.failed-to-start-notificator stack: ' + err.stack);
                reject();
            });
            ...
            scriptsMgr.start().then(function () {
                ...
                if (err && err.stack) logger.error('runtime.failed-to-start-scripts stack: ' + err.stack);
                reject();
            });
            // start jobs manager
            jobsMgr.start().then(function () {
                ...
                if (err && err.stack) logger.error('runtime.failed-to-start-jobs stack: ' + err.stack);
                reject();
            });
        }).catch(function (err) {
            logger.error('runtime.failed-to-start: ' + err);
            logger.error('runtime.failed-to-start stack: ' + (err && err.stack ? err.stack : 'no stack'));
            reject();
        });
    });
}
    ...
    return new Promise(function (resolve, reject) {
        try {
            if (cmd === project.ProjectDataCmdType.SetDevice) {
                ...
                alarmsMgr.reset();
            } else if (cmd === project.ProjectDataCmdType.SetDeviceTags) {
                // 仅保存标签配置，不重启采集（用户手动点击重启按钮时才重启）
                alarmsMgr.reset();
