...
try {
    ...
    // Always merge to ensure all default fields are present in user settings
...
try {
    ...
    if (fs.existsSync(userSettingsFile)) {
        ...
        if (mysettings.dbType) {
            settings.dbType = mysettings.dbType;
        }
        if (mysettings.mysql) {
            settings.mysql = utils.deepMerge(settings.mysql || {}, mysettings.mysql);
        }
        if (mysettings.mssql) {
            settings.mssql = utils.deepMerge(settings.mssql || {}, mysettings.mssql);
        }
