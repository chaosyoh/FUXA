...
module.exports = {
  ...
  app: function () {
    ...
    prjApp.get("/api/project", secureFnc, function (req, res) {
      ...
      runtime.project
        .getProject(req.userId, permission)
        .then((result) => {
     ...
     * Take from project storage and reply
     */
    prjApp.get("/api/projectVersion", secureFnc, function (req, res) {
      res.json({ timestamp: runtime.project.getTimeStamp() });
    ...
    prjApp.post("/api/projectData", secureFnc, function (req, res, next) {
      ...
      if (res.statusCode === 403) {
        ...
        runtime.project
          .setProjectData(req.body.cmd, req.body.data)
          .then((setres) => {
    ...
    prjApp.post("/api/device", secureFnc, function (req, res, next) {
      ...
      if (res.statusCode === 403) {
        ...
        runtime.project
          .setDeviceProperty(req.body.params)
          .then(function (data) {
            res.end();
          })
          .catch(function (err) {
    ...
    /**
     * POST Device property
     * Set to project storagereq.body
     */
    prjApp.post("/api/getTagValues", secureFnc, function (req, res, next) {
      let tagIds = req.body;
      if (!Array.isArray(tagIds)) {
        res
          .status(400)
          .json({ error: "invalid_tags", message: "Invalid tags." });
        return;
      }
      let advs = runtime.devices.getDevicesValues();
        let tags = {}
        for (const tagId of tagIds) {
            for (const deviceId in advs) {
                if(advs[deviceId].hasOwnProperty(tagId)){
                    tags[tagId] = advs[deviceId][tagId].value;
                }
            }
        }
      // console.log(runtime.devices.getDevicesValues())
      res.json(tags);
      res.end();
    });
     ...
     * GET Device
     */
    prjApp.get("/api/getDevices", secureFnc, function (req, res, next) {
      let advs = runtime.devices.getDevicesValues();   
      // console.log(runtime.devices.getDevicesValues())
      res.json(runtime.project.getDevices());
      res.end();
    });
