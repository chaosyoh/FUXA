...
module.exports = {
    ...
    app: function() {
            ...
            const permission = checkGroupsFnc(req);
			 if (res.statusCode === 403) {
                ...
                try {
                    ...
                    var result = runtime.alarmsMgr.getAlarmsValues(filter, permission).filter(x=>x.type!='info');
         ...
         * Take from alarms storage and reply
         */
        alarmsApp.get("/api/getAlarms", secureFnc, function(req, res) {
            try {
                var filter = req.query.filter ? JSON.parse(req.query.filter) : null;
                var result = runtime.alarmsMgr.getAlarmsValues(filter);  
                if (result) {
                    let alarms = []
                    for(const x of result){
                        let level = x.type == 'highhigh'?'紧急' :x.type == 'high'?'重要':x.type=='low'?'一般':'信息';
                        let time = new Date(x.ontime);
                        alarms.push({
                            time:time.toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' }),
                            equipment:x.group,
                            description:x.text,
                            level:level                    
                        })
