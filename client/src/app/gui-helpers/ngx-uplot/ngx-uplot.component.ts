...
export class NgxUplotComponent implements OnInit, OnDestroy {
    ...
    private getFullTimeFormatWithMs() {
        if (this.options && this.options.timeFormat === 'hh_mm_ss_AA') {
            return '{h}:{mm}:{ss} {AA}';
        }
        return '{HH}:{mm}:{ss}';
    ...
    sampleSerie = [
        {
            value: (self, rawValue) => (rawValue != null) ? this.fmtDate(new Date(rawValue * 1e3)) : ''
        },
        {
    ...
    init(options?: ChartOptions, rawData?: boolean) {
        ...
        if (this.options.series.length > 0) {
            this.options.series[0].value = (self, rawValue) => (rawValue != null) ? (this.rawData ? rawValue : this.fmtDate(new Date(rawValue * 1e3))) : '';
    ...
    paintLegendCuror = (u: any, overlay, bLeft, bTop, bound) => {
        ...
        for (let i = 1; i < u.series.length; i++) {
            ...
            series = series + `<div class="ut-serie"><div class="ut-marker" style="background: ${strokeColor}"></div>${u.series[i].label}: <div class="ut-value">${value}</div></div>`;
        }
    ...
    paintLegendMarkers = (u: any) => {
        ...
        for (let i = 1; i < u.series.length && i < rows.length; i++) {
            ...
            if (u.series[i].show) {
                marker.style.background = String(strokeColor || '');
                marker.style.border = 'none';
                marker.textContent = '\u2713';
            } else {
                marker.style.background = 'transparent';
                marker.style.border = `2px solid ${strokeColor || 'rgba(0,0,0,0.3)'}`;
                marker.textContent = '';
            }

            // Override colors for scatter/points-only series
            if (u.series[i].lineInterpolation === this.lineInterpolations?.none && u.series[i].points) {
                let pointStroke = u.series[i].points._stroke ?? strokeColor;
                let pointFill = u.series[i].points._fill ?? u.series[i]._fill;
                if (u.series[i].show) {
                    marker.style.background = String(pointFill || pointStroke || '');
                    if (pointStroke) {
                        marker.style.border = `2px solid ${pointStroke}`;
                    }
                } else {
                    marker.style.background = 'transparent';
                    marker.style.border = `2px solid ${pointStroke || 'rgba(0,0,0,0.3)'}`;
                }
            }
        }
    };
    ...
    _proximityIndex(self, seriesIdx, hoveredIdx, cursorXVal) {
        // console.log(self, seriesIdx, hoveredIdx, cursorXVal);
...
export interface ChartOptions extends NgxOptions {
    ...
    /** Map of axis index to visibility/label overrides, applied before uPlot init */
    axisConfig?: { [axisIndex: number]: { show?: boolean; label?: string } };
