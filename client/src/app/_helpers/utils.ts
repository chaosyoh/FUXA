...
export class Utils {
    ...
    static searchVariableIdsWithPath(jsonData: any, attributeName: string): {value: string, path: string}[] {
        const result: {value: string, path: string}[] = [];
        function search(data: any, currentPath: string): void {
            if (Array.isArray(data)) {
                for (let i = 0; i < data.length; i++) {
                    search(data[i], `${currentPath}[${i}]`);
                }
            } else if (typeof data === 'object' && data !== null) {
                if (data.hasOwnProperty(attributeName) && data[attributeName]) {
                    result.push({ value: data[attributeName], path: currentPath ? `${currentPath}.${attributeName}` : attributeName });
                }
                for (const key in data) {
                    if (data.hasOwnProperty(key) && key !== attributeName) {
                        const nextPath = currentPath ? `${currentPath}.${key}` : key;
                        search(data[key], nextPath);
                    }
                }
            }
        }
    ...
    static formatValue(value: string, format: string): string {
        try {
            if (Utils.isNumeric(value)) {
                // numeral.format() with no/empty format uses its default which
                // rounds to integer — only apply formatting when an explicit
                // format string is provided so raw float values are preserved.
                if (format) {
