...
import { DocAlignType, DocProfile, ViewFolder, ViewProperty, ViewType } from '../../_models/hmi';
...
export class ViewPropertyComponent implements OnInit, OnDestroy {
    ...
    onOkClick(): void {
        ...
        this.data.folderId = this.formGroup.controls.folderId.value || undefined;
    ...
    getFolderPath(folderId: string): string {
        if (!folderId || !this.data.viewFolders) { return ''; }
        const parts: string[] = [];
        let current = folderId;
        while (current) {
            const folder = this.data.viewFolders.find(f => f.id === current);
            if (!folder) { break; }
