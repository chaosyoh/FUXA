...
import { Hmi, View, GaugeSettings, SelElement, LayoutSettings, ViewType, ISvgElement, GaugeProperty, DocProfile, ViewFolder } from '../_models/hmi';
...
import { ChangeReferencesComponent, ChangeRefsData, ReferenceItem } from './change-references/change-references.component';
...
export class EditorComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    @ViewChild('tabContextMenuTrigger', {static: false}) tabContextMenuTrigger: MatMenuTrigger;
    ...
    /** Views currently opened in the tab navigation bar */
    openedViews: View[] = [];
    /** Position (in viewport pixels) used by the tab right-click context menu anchor */
    tabContextMenuPosition = { x: '0px', y: '0px' };
    /** The view targeted by the current tab right-click action */
    tabContextMenuView: View = null;
    shapesGrps = [];
    private gaugesRef = {};
    ...
    private loadHmi(firstTime = false) {
        ...
        if (!this.hmi.viewFolders) {
            this.hmi.viewFolders = [];
        }
        // check new hmi
    ...
    onAddDoc() {
        let dialogRef = this.dialog.open(ViewPropertyComponent, {
            ...
            data: <ViewPropertyType & { newView: boolean}> {
                ...
                viewFolders: this.hmi.viewFolders || [],
                newView: true
            }
        });
    ...
    onAddFolder() {
        let existFolderNames = (this.hmi.viewFolders || []).map(f => f.name);
        let dialogRef = this.dialog.open(EditNameComponent, {
            disableClose: true,
            position: { top: '60px' },
            data: <EditNameData>{
                title: this.translateService.instant('editor.folder-add'),
                name: '',
                exist: existFolderNames
            }
        });
        dialogRef.afterClosed().subscribe(result => {
            if (result && result.name) {
                ...
                this.hmi.viewFolders.push(folder);
                this.projectService.setViewFolders(this.hmi.viewFolders);
            }
        });
    }

    onViewFoldersChanged(viewFolders: ViewFolder[]) {
        this.hmi.viewFolders = viewFolders;
        this.projectService.setViewFolders(this.hmi.viewFolders);
    }
    ...
    onSelectView(view: View, force = true) {
        ...
        this.addOpenedTab(this.currentView);
    }

    /**
     * Add the view to the tab navigation bar if not already opened.
     */
    private addOpenedTab(view: View) {
        if (!view) { return; }
        if (!this.openedViews.some(v => v.id === view.id)) {
            this.openedViews.push(view);
        }
    }

    /**
     * Filter out tabs whose underlying view no longer exists in hmi.views.
     */
    getOpenedTabs(): View[] {
        if (!this.hmi?.views) { return []; }
        this.openedViews = this.openedViews.filter(v => this.hmi.views.some(hv => hv.id === v.id));
        return this.openedViews;
    }

    /**
     * Click handler for a tab: switch to the corresponding view.
     */
    onSelectTab(view: View) {
        if (!view) { return; }
        this.onSelectView(view, false);
    }

    /**
     * Close a tab. If the closed tab is the current view, switch to a neighbor tab.
     */
    onCloseTab(view: View, event?: MouseEvent) {
        if (event) {
            event.stopPropagation();
            event.preventDefault();
        }
        const idx = this.openedViews.findIndex(v => v.id === view.id);
        if (idx < 0) { return; }
        const isActive = this.currentView?.id === view.id;
        this.openedViews.splice(idx, 1);
        if (isActive && this.openedViews.length > 0) {
            const next = this.openedViews[idx] || this.openedViews[idx - 1];
            if (next) {
                this.onSelectView(next, false);
            }
        }
    }

    /** Returns true if the given view is currently active. */
    isTabActive(view: View): boolean {
        return !!view && this.currentView?.id === view.id;
    }

    /**
     * Open the tab right-click context menu near the cursor.
     */
    onTabContextMenu(event: MouseEvent, view: View) {
        if (event) {
            event.preventDefault();
            event.stopPropagation();
        }
        if (!this.tabContextMenuTrigger) {
            return;
        }
        this.tabContextMenuView = view;
        this.tabContextMenuPosition = { x: event.clientX + 'px', y: event.clientY + 'px' };
        // Close any previously opened menu before opening at the new position
        this.tabContextMenuTrigger.closeMenu();
        setTimeout(() => this.tabContextMenuTrigger.openMenu(), 0);
    }

    /** Close the tab targeted by the right-click menu (i.e. close current). */
    onCloseContextTab() {
        if (this.tabContextMenuView) {
            this.onCloseTab(this.tabContextMenuView, null);
        }
    }

    /** Close every opened tab except the one targeted by the right-click menu. */
    onCloseOtherTabs() {
        const target = this.tabContextMenuView;
        if (!target) { return; }
        this.openedViews = this.openedViews.filter(v => v.id === target.id);
        if (this.currentView?.id !== target.id) {
            this.onSelectView(target, false);
        }
    }

    /** Close every opened tab. */
    onCloseAllTabs() {
        this.openedViews = [];
    }
    ...
    private loadPanelState() {
        ...
        if (ps) {
            ...
            // Migration: bump legacy default view-panel height (200) up to the new default (500)
            // so existing users immediately see more views without breaking custom heights.
            if (this.panelsState.panelViewHeight === 200) {
                this.panelsState.panelViewHeight = 500;
            }
        }
    }
    ...
    openEditGauge(settings, callback) {
        ...
        } else if (dlgType === GaugeDialogType.Table || dlgType === GaugeDialogType.Panel || dlgType === GaugeDialogType.AlarmList) {
            this.gaugeDialog.type = dlgType;
            this.gaugeDialog.data = {
                settings: tempsettings, dlgType: dlgType, names: names
            };
    ...
    batchReplaceTags() {
        ...
        const tagsIds = new Set();
        const elesSelected = this.winRef.nativeWindow.svgEditor.getSelectedElements();
        if (elesSelected?.length && elesSelected[0]) {
            // 有选中组件（包含多选）- 只处理选中的组件
            for (let i = 0; i < elesSelected.length; i++) {
                const eleIdsAndTypes = Utils.getInTreeIdAndType(elesSelected[i]);
                if (eleIdsAndTypes?.length) {
                    for (let j = 0; j < eleIdsAndTypes.length; j++) {
                        let gaSrc: GaugeSettings = this.searchGaugeSettings(eleIdsAndTypes[j]);
                        const variablesIds = Utils.searchValuesByAttribute(gaSrc, 'variableId');
                        if (variablesIds?.length) {
                            gaugesSettings.push(gaSrc);
                            variablesIds.forEach(id => {
            ...
            // 没有选中组件 - 处理所有组件
            const allItems = Object.values(this.currentView.items);
            for (let i = 0; i < allItems.length; i++) {
                const gaSrc = allItems[i];
                const variablesIds = Utils.searchValuesByAttribute(gaSrc, 'variableId');
                if (variablesIds?.length) {
                    gaugesSettings.push(gaSrc);
                    variablesIds.forEach(id => {
    ...
    openChangeReferences() {
            ...
            for (let i = 0; i < elesSelected.length; i++) {
                ...
                if (eleIdsAndTypes?.length) {
                    for (let j = 0; j < eleIdsAndTypes.length; j++) {
                        let gaSrc: GaugeSettings = this.searchGaugeSettings(eleIdsAndTypes[j]);
                        if (gaSrc) {
                            const varIds = Utils.searchVariableIdsWithPath(gaSrc, 'variableId');
                            if (varIds?.length) {
                                gaugesSettings.push(gaSrc);
                                this.buildReferenceItems(gaSrc, varIds, references);
                            }
                        }
                    }
                }
        ...
        const devices = Object.values(this.projectService.getDevices());
        const dialogRef = this.dialog.open(ChangeReferencesComponent, {
            position: { top: '60px' },
            data: <ChangeRefsData>{
                devices: devices,
        ...
        dialogRef.afterClosed().subscribe((result: { srcId: string, destId: string }[]) => {
            if (result?.length) {
                gaugesSettings.forEach(gaSettings => {
                    result.forEach(ref => {
                        Utils.changeAttributeValue(gaSettings, 'variableId', ref.srcId, ref.destId);
                    });
                });
                this.saveView(this.currentView);
            }
    ...
    private buildReferenceItems(gaSrc: GaugeSettings, varIds: { value: string, path: string }[], references: ReferenceItem[]) {
        for (const varId of varIds) {
            const tag = this.projectService.getTagFromId(varId.value);
            const device = this.projectService.getDeviceFromTagId(varId.value);
            references.push({
                gaugeId: gaSrc.id,
                gaugeName: gaSrc.name || gaSrc.id,
                variableId: varId.value,
                tagName: tag?.name || '',
                deviceName: device?.name || '',
                path: varId.path,
                pathDisplay: this.buildPathDisplay(gaSrc, varId.path)
            });
        }
    }

    private buildPathDisplay(gaSrc: GaugeSettings, path: string): string {
        const cleanPath = path.replace(/^property\./, '');
        if (cleanPath === 'variableId') {
            return this.translateService.instant('editor.change-refs-path-display');
        }
        const actionsMatch = cleanPath.match(/^actions\[(\d+)\]\.variableId$/);
        if (actionsMatch) {
            const idx = parseInt(actionsMatch[1], 10);
            const actions = gaSrc.property?.actions;
            let actionTypeName = '';
            if (actions && actions[idx] && actions[idx].type) {
                actionTypeName = this.translateService.instant(actions[idx].type);
            }
            const actionLabel = this.translateService.instant('editor.change-refs-path-action');
            return actionTypeName ? `${actionLabel}\\${actionTypeName}` : actionLabel;
        }
        const eventsMatch = cleanPath.match(/^events\[(\d+)\]\.actoptions\.(?:variable\.)?variableId$/);
        if (eventsMatch) {
            const idx = parseInt(eventsMatch[1], 10);
            const events = gaSrc.property?.events;
            let eventTypeName = '';
            if (events && events[idx] && events[idx].type) {
                eventTypeName = this.translateService.instant(events[idx].type);
            }
            const eventLabel = this.translateService.instant('editor.change-refs-path-event');
            return eventTypeName ? `${eventLabel}\\${eventTypeName}` : eventLabel;
        }
        return cleanPath;
    }
