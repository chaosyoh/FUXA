...
import { ProjectDataCmdType } from '../../_models/project';
...
export class SetupComponent {
    ...
    onLayoutConfig() {
        this.onNoClick();
        let templayout = null;
        let hmi = this.projectService.getProject().hmi;
        if (hmi.layout) {
     ...
     * edit the layout property of views: menu, header
     */
    onMobileLayoutConfig() {
        this.onNoClick();
        let templayout = null;
        let hmi = this.projectService.getProject().hmi;
        if (hmi.mobileLayout) {
            templayout = JSON.parse(JSON.stringify(hmi.mobileLayout));
        }
        if (templayout && templayout.showdev !== false) {
            templayout.showdev = true;
        }
        ...
        dialogRef.afterClosed().subscribe(result => {
            if (result) {
                hmi.mobileLayout = JSON.parse(JSON.stringify(result.layout));
                this.projectService.setLayout(hmi.mobileLayout,ProjectDataCmdType.MobileLayout);
            }
        });
    }
