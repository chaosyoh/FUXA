...
export class ChartConfigComponent implements OnInit {
    ...
    @ViewChild('lineMenuTrigger', {static: false}) contextMenuTrigger: MatMenuTrigger;
    ...
    editChartLine(line: ChartLine) {
        ...
        dialogRef.afterClosed().subscribe((result: ChartLine) => {
            if (result) {
                ...
                line.yaxisLabel = result.yaxisLabel;
                line.yaxisShow = result.yaxisShow;
    ...
    onLineContextMenu(event: MouseEvent, line: ChartLine, index: number) {
        event.preventDefault();
        event.stopPropagation();
        this.contextLineIndex = index;
        this.contextMenuPosition = { x: event.clientX, y: event.clientY };
        this.contextMenuTrigger.openMenu();
    }

    onLineMoveTop() {
        if (this.contextLineIndex > 0) {
            const lines = this.selectedChart.lines;
            const [item] = lines.splice(this.contextLineIndex, 1);
            lines.unshift(item);
        }
    }
    ...
    onLineMoveBottom() {
        const lines = this.selectedChart.lines;
        if (this.contextLineIndex < lines.length - 1) {
            const [item] = lines.splice(this.contextLineIndex, 1);
            lines.push(item);
        }
    }
    ...
    onLineMoveUp() {
        if (this.contextLineIndex > 0) {
            const lines = this.selectedChart.lines;
            const temp = lines[this.contextLineIndex];
            lines[this.contextLineIndex] = lines[this.contextLineIndex - 1];
            lines[this.contextLineIndex - 1] = temp;
        }
    }
    ...
    onInsertChartLine() {
        ...
        dialogRef.afterClosed().subscribe((result) => {
            if (result) {
                ...
                let insertIndex = this.contextLineIndex;
                tagsId.forEach(id => {
                    let device = DevicesUtils.getDeviceFromTagId(this.data.devices, id);
                    let tag = DevicesUtils.getTagFromTagId([device], id);
                    if (tag) {
                        let exist = this.selectedChart.lines.find(line => line.id === tag.id);
                        if (!exist) {
                            ...
                            this.selectedChart.lines.splice(insertIndex, 0, newLine);
                            insertIndex++;
                        }
                    }
                });
