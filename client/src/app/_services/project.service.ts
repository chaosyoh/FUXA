...
export class ProjectService {
    ...
    exportDevices(type: string) {
        ...
        if (type === 'csv') {
            ...
            const exportData: any = {
                devices: devices,
                deviceFolders: this.getDeviceFolders() || {}
            };
    ...
    importDeviceFolders(deviceFolders: { [id: string]: any }) {
        if (!deviceFolders) return;
        for (const folder of Object.values(deviceFolders)) {
            if (folder && folder.id && folder.name) {
                this.setDeviceFolder(folder);
            }
        }
    }

    exportDevicesXls() {
        console.log('exportDevicesXls');
        this.resewbApiService.exportDevicesXls().subscribe({
            next: (blob: Blob) => {
                const name = this.projectData.name || 'fuxa';
                FileSaver.saveAs(blob, `${name}-devices.xlsx`);
            },
            error: (err) => {
                console.error('exportDevicesXls error:', err);
                this.notifyError('msg.export-devices-error');
            }
        });
    }

    importDevicesXls(file: File, isTemplate: boolean) {
        this.resewbApiService.importDevicesXls(file, isTemplate).subscribe({
            next: (result: any) => {
                if (result && result.devices) {
                    this.importDevices(result.devices);
                    if (result.deviceFolders) {
                        this.importDeviceFolders(result.deviceFolders);
                    }
                    setTimeout(() => { this.onRefreshProject(); }, 2000);
                } else {
                    this.notifyError('msg.import-devices-error');
                }
            },
            error: (err) => {
                console.error('importDevicesXls error:', err);
                this.notifyError('msg.import-devices-error');
            }
        });
    }

    importKepserver(file: File) {
        this.resewbApiService.importKepserver(file).subscribe({
            next: (result: any) => {
                if (result && result.devices) {
                    this.importDevices(result.devices);
                    setTimeout(() => { this.onRefreshProject(); }, 2000);
            ...
            error: (err) => {
                console.error('importKepserver error:', err);
                this.notifyError('msg.import-devices-error');
            }
        });
    }
    ...
    setDeviceTags(device: Device) {
        this.projectData.devices[device.id] = device;
        this.storage.setServerProjectData(ProjectDataCmdType.SetDeviceTags, device, this.projectData).subscribe(result => {
        }, err => {
            console.error(err);
            this.notifySaveError(err);
        });
     ...
     * Get device folders
     */
    getDeviceFolders(): { [id: string]: any } {
        return this.projectData.deviceFolders || {};
    }

    /**
     * Set (add/update) a device folder and save
     */
    setDeviceFolder(folder: any) {
        if (!this.projectData.deviceFolders) {
            this.projectData.deviceFolders = {};
        }
        this.projectData.deviceFolders[folder.id] = folder;
        this.storage.setServerProjectData(ProjectDataCmdType.SetDevice, folder, this.projectData).subscribe(() => {
        }, err => {
            console.error(err);
            this.notifySaveError(err);
        });
    ...
    /**
     * Remove a device folder and save
     */
    removeDeviceFolder(folder: any) {
        if (this.projectData.deviceFolders) {
            delete this.projectData.deviceFolders[folder.id];
        }
        this.storage.setServerProjectData(ProjectDataCmdType.SetDevice, folder, this.projectData).subscribe(() => {
        }, err => {
            console.error(err);
    ...
    getViewFolders(): ViewFolder[] {
        return (this.projectData?.hmi?.viewFolders) ? this.projectData.hmi.viewFolders : [];
    }

    setViewFolders(viewFolders: ViewFolder[]) {
        this.projectData.hmi.viewFolders = viewFolders;
        this.storage.setServerProjectData(ProjectDataCmdType.ViewFolders, viewFolders, this.projectData).subscribe(result => {
        }, err => {
            console.error(err);
            this.notifySaveError(err);
        });
    ...
    getHmi() {
        ...
        let hmi;
        if(this.ready && this.projectData){
            hmi = {
                views : this.projectData.hmi.views,
                layout : isMobile? this.projectData.hmi.mobileLayout : this.projectData.hmi.layout,
                viewFolders: this.projectData.hmi.viewFolders || []
            };
        }
        else {
            hmi = null;
    ...
    setLayout(layout: LayoutSettings, cmdType: ProjectDataCmdType = ProjectDataCmdType.HmiLayout) {
        if(cmdType === ProjectDataCmdType.HmiLayout){
            this.projectData.hmi.layout = layout;
        }
        else if(cmdType === ProjectDataCmdType.MobileLayout){
            this.projectData.hmi.mobileLayout = layout;
        }
        this.saveLayout(layout,cmdType);
    }


    setLayoutTheme(theme: string) {
        this.projectData.hmi.layout.theme = theme;
        this.saveLayout(this.projectData.hmi.layout);
    }
    ...
    saveLayout(layout: LayoutSettings, cmdType: ProjectDataCmdType = ProjectDataCmdType.HmiLayout) {
        this.storage.setServerProjectData(cmdType, layout, this.projectData).subscribe(result => {
        }, err => {
            console.error(err);
            this.notifySaveError(err);
        });
    ...
    /**
     * Check to add, update or remove system Tags, example connection status to add in device FUXA server.
     * - Creates a connection status tag for each non-FuxaServer device that doesn't have one
     * - Updates tag name/label when a device has been renamed
     * - Removes orphaned connection status tags for devices that no longer exist
     */
    checkSystemTags() {
        ...
        if (fuxaServer) {
            ...
            // Remove orphaned connection status tags (device no longer exists)
            Object.values(fuxaServer.tags).forEach((tag: Tag) => {
                if (tag.sysType === TagSystemType.deviceConnectionStatus && tag.memaddress && !deviceIds.has(tag.memaddress)) {
                    delete fuxaServer.tags[tag.id];
                    changed = true;
                }
            });
            // Create or update connection status tag for each device
            devices.forEach((device: Device) => {
                let connTag = Object.values(fuxaServer.tags).find((tag: Tag) => tag.sysType === TagSystemType.deviceConnectionStatus && tag.memaddress === device.id);
                const expectedName = device.name + ' Connection Status';
                if (!connTag) {
                    let tag = new Tag(Utils.getGUID(TAG_PREFIX));
                    tag.name = expectedName;
                    tag.label = expectedName;
                    ...
                    changed = true;
                } else if (connTag.name !== expectedName) {
                    // Device was renamed — update tag name and label
                    connTag.name = expectedName;
                    connTag.label = expectedName;
                    changed = true;
                }
            });
            if (changed) {
