...
export class HmiService {
    ...
    public initSocket(token: string = null) {
        ...
        console.log('init socket: ', this.endPointConfig);
        ...
        // project updated notification from backend
        this.socket.on(IoEventTypes.PROJECT_UPDATED, (message) => {
            console.log('[HmiService] Received PROJECT_UPDATED:', message);
    ...
    public restartDevice(deviceId: string) {
        if (this.socket) {
            ...
            this.socket.emit(IoEventTypes.DEVICE_RESTART, msg);
        }
    }
    //#endregion
    ...
    public notifyProjectUpdate() {
        if (this.socket) {
            console.log('[HmiService] Emitting PROJECT_NOTIFY_UPDATE via socket');
            this.socket.emit(IoEventTypes.PROJECT_NOTIFY_UPDATE);
        } else {
            console.warn('[HmiService] Cannot notify project update: socket is not connected');
        }
    }
