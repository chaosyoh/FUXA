...
export class FuxaViewComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    private instanceId: string = Utils.getGUID('fv_');
    ...
    ngAfterViewInit() {
        ...
        // }
        try {
            this.gaugesManager.emitBindedSignals(this.instanceId);
        } catch (err) {
            console.error(err);
        }
    }
    ...
    ngOnDestroy() {
        try {
            ...
            this.gaugesManager.unbindGauge(this.instanceId);
            HtmlVideoComponent.destroyAll();
        ...
        if (view && view.items) {
            ...
            for (let key in items) {
                ...
                try {
                    ...
                    this.gaugesManager.bindGauge(gauge, this.instanceId, items[key], sourceTags,
                        (gaToBindMouseEvents) => {
                    ...
                    if (items[key].property) {
                        ...
                        if (variables.length) {
                            let svgeles = this.getLocalSvgElements(gaugeSetting.id);
    ...
    protected handleSignal(sig) {
        if (sig.value !== undefined) {
            try {
                ...
                let gas = this.gaugesManager.getGaugeSettings(this.instanceId, sig.id);
                if (gas) {
    ...
    private onBindMouseEvents(ga: GaugeSettings) {
        ...
        let svgele = this.getLocalSvgElement(ga.id);
        let clickTimeout;
    ...
    public runEvents(self: any, ga: GaugeSettings, ev: any, events: any) {
        // Layer 1 逻辑层阻断：即使 CSS 被篡改，disabled 状态下也不执行任何事件
        if (self.mapGaugeStatus?.[ga.id]?.disabled) { return; }
        // Layer 2: 未登录用户点击需授权控件 → 弹出登录对话框
        console.log('[DEBUG] runEvents called for gauge:', ga.id, '_requiresLogin:', ga?.property?._requiresLogin, 'events:', events?.length);
        ...
        for (let i = 0; i < events.length; i++) {
            ...
            } else if (eventTypes.indexOf(GaugeEventActionType.onSetControlProp) === actindex) {
                self.onSetControlProp(events[i]);
            }
        }
    }

    /**
     * Check if control requires login. Opens login dialog if user is not authenticated.
     * @returns true if event should be intercepted (blocked)
     */
    private checkRequiresLogin(ga: GaugeSettings): boolean {
        if (!ga?.property?._requiresLogin) {
            return false;
        }
    ...
    /**
     * Open login dialog (prevents duplicate dialogs).
     */
    private openLoginDialog(): void {
        ...
        this.toastr.warning('无权限操作，请先登录', '', {
            timeOut: 3000,
            closeButton: true,
            disableTimeOut: true,
    ...
    /**
     * Bind capture-phase click handler to intercept clicks inside foreignObject
     * elements marked with data-requires-login.
     */
        ...
        if (htmlevent.type === 'key-enter') {
            ...
            if (this.hmi?.layout?.inputdialog === 'true') {
                ...
                htmlevent.dom.onblur = function(ev) {
                    ...
                    let svgeles = self.getLocalSvgElements(htmlevent.ga.id);
    ...
    private getLocalSvgElements(svgid: string) {
        let ele = this.dataContainer.nativeElement.getElementsByTagName('svg');
        let result = [];
        for (let i = 0; i < ele.length; i++) {
            let svgItems = ele[i].getElementById(svgid);
            if (svgItems) {
    ...
    private getLocalSvgElement(svgid: string) {
        let ele = this.dataContainer.nativeElement.getElementsByTagName('svg');
        for (let i = 0; i < ele.length; i++) {
            let svgItems = ele[i].getElementById(svgid);
            if (svgItems) {
    ...
    onOpenCard(id: string, event: PointerEvent | TouchEvent | any, viewref: string, options: any = {}) {
        ...
        if (options.positionType === GaugeEventPositionType.custom ||
            (!options.positionType && (Utils.isNumeric(options.left) || Utils.isNumeric(options.top)))) {
            // Custom position: use x/y + relative from mouse/window
            ...
            // Fixed position (default): calculate based on viewport and card dimensions
            const vw = window.innerWidth;
            const vh = window.innerHeight;
            const cw = view.profile.width || 800;
            const ch = view.profile.height || 600;
            const fixV = options.fixedPositionV || GaugeEventFixedPositionV.center;
            const fixH = options.fixedPositionH || GaugeEventFixedPositionH.center;
            switch (fixH) {
                case GaugeEventFixedPositionH.left:
                    card.x = 0;
                    break;
                case GaugeEventFixedPositionH.right:
                    card.x = vw - cw;
                    break;
                default: // center
                    card.x = Math.round((vw - cw) / 2);
                    break;
            }
            switch (fixV) {
                case GaugeEventFixedPositionV.top:
                    card.y = 0;
                    break;
                case GaugeEventFixedPositionV.bottom:
                    card.y = vh - ch;
                    break;
                default: // center
                    card.y = Math.round((vh - ch) / 2);
                    break;
            }
        }
    ...
    onSetControlProp(event: GaugeEvent): void {
        const targetId = event.actoptions?.targetId || event.actparam;
        if (!targetId) { return; }
        const targetEle = document.getElementById(targetId);
        if (!targetEle) { return; }
        const properties: ControlPropertyItem[] = event.actoptions?.properties || [];
        for (const prop of properties) {
            switch (prop.name) {
                case 'text':
                    this.setControlText(targetEle, prop.value);
                    break;
                case 'visibility':
                    this.setControlVisibility(targetEle, prop.value);
                    break;
                case 'backgroundColor':
                    this.setControlStyle(targetEle, 'backgroundColor', prop.value);
                    break;
                case 'color':
                    this.setControlStyle(targetEle, 'color', prop.value);
                    break;
                case 'enabled':
                    this.setControlEnabled(targetEle, prop.value);
                    break;
                case 'fontSize':
                    this.setControlStyle(targetEle, 'fontSize', prop.value + 'px');
                    break;
                case 'border':
                    this.setControlStyle(targetEle, 'border', prop.value);
                    break;
            }
        }
    }

    private setControlText(ele: HTMLElement, text: string): void {
        // Try HTML button (HtmlButtonComponent prefix B-HXB_)
        let htmlButton = Utils.searchTreeStartWith(ele, 'B-HXB_');
        if (htmlButton) {
            htmlButton.innerHTML = text || '<span>&nbsp;</span>';
            return;
        }
        // Try SVG <text> element
        let textEle = ele.querySelector('text');
        if (textEle) {
            textEle.textContent = text;
            return;
        }
        // Fallback: set textContent on the element itself
        ele.textContent = text;
    }

    private setControlVisibility(ele: HTMLElement, value: string): void {
        ele.style.display = (value === 'hidden') ? 'none' : '';
    }

    private setControlStyle(ele: HTMLElement, styleProp: string, styleValue: string): void {
        // Try known inner element prefixes for HTML controls
        const innerPrefixes = ['B-HXB_', 'I-HXI_', 'S-HXS_', 'T-HXT_', 'D-OXC_'];
        for (const prefix of innerPrefixes) {
            let inner = Utils.searchTreeStartWith(ele, prefix);
            if (inner) {
                (inner.style as any)[styleProp] = styleValue;
                return;
            }
        }
        // Fallback: apply to the root element
        (ele.style as any)[styleProp] = styleValue;
    }

    private setControlEnabled(ele: HTMLElement, value: string): void {
        if (value === 'disabled') {
    ...
    getCardHeight(card: CardModel) {
        const headerHeight = card.disableDefaultClose ? 0 : 32;
        return parseInt(card.height as any) + headerHeight;
    }

    onToggleFullscreen(card: CardModel) {
...
export class CardModel {
    ...
    private _savedRect: { x: number, y: number, width: number, height: number } = null;
    ...
    toggleFullscreen() {
        if (this.fullscreen) {
            // Restore
            if (this._savedRect) {
                this.x = this._savedRect.x;
                this.y = this._savedRect.y;
                this.width = this._savedRect.width;
                this.height = this._savedRect.height;
            }
            this.fullscreen = false;
        } else {
            // Save & expand
            this._savedRect = { x: this.x, y: this.y, width: this.width, height: this.height };
            this.x = 0;
            this.y = 0;
