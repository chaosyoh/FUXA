...
export class FlexInputComponent implements OnInit {
    ...
    @Input() default: any;
    @Input() withVariableBinding = false;
    ...
    ngOnInit() {
        if (!this.ranges) {
            ...
            if (this.isWithStep() || this.isWithImageStep()) {
                ip.type = this.type;
                ip.min = 1;
                ip.max = 1;
    ...
    isWithImageStep() {
        return this.propertyType === PropertyType.imagestep;
    }
    ...
    onMinChanged(range: GaugeRangeProperty, event) {
        range.minVariableId = event?.variableId || null;
        const v = parseFloat(event?.variableValue);
        if (!Number.isNaN(v)) {
            range.min = v;
        }
    }

    onMaxChanged(range: GaugeRangeProperty, event) {
        range.maxVariableId = event?.variableId || null;
        const v = parseFloat(event?.variableValue);
        if (!Number.isNaN(v)) {
            range.max = v;
        }
    ...
    onImageUpload(item: GaugeRangeProperty, event: any) {
