...
import {
    ...
    GaugeEventFixedPositionH,
    GaugeEventFixedPositionV,
    GaugeEventPositionType,
...
export class FlexEventComponent implements OnInit {
    ...
    positionType = GaugeEventPositionType;
    fixedPositionV = GaugeEventFixedPositionV;
    fixedPositionH = GaugeEventFixedPositionH;
    ...
    eventSetControlProp = Utils.getEnumKey(GaugeEventActionType, GaugeEventActionType.onSetControlProp);

    viewControls: { id: string, name: string }[] = [];
    controlPropertyTypes = [
        { key: 'text', label: 'gauges.property-event-ctrl-prop-text' },
        { key: 'visibility', label: 'gauges.property-event-ctrl-prop-visibility' },
        { key: 'backgroundColor', label: 'gauges.property-event-ctrl-prop-bgcolor' },
        { key: 'color', label: 'gauges.property-event-ctrl-prop-textcolor' },
        { key: 'enabled', label: 'gauges.property-event-ctrl-prop-enabled' },
        { key: 'fontSize', label: 'gauges.property-event-ctrl-prop-fontsize' },
        { key: 'border', label: 'gauges.property-event-ctrl-prop-border' },
    ];
    ...
    ngOnInit() {
        ...
        this.viewControls = Object.values(this.data.view?.items ?? {})
            .filter((item: any) => item.id !== this.data.settings?.id)
    ...
    withSetControlProp(action: string): boolean {
        return action === this.eventSetControlProp;
    }

    onActionChange(event: GaugeEvent): void {
        event.actparam = '';
        if (event.action === this.eventSetControlProp) {
            event.actoptions = event.actoptions || {};
            event.actoptions.targetId = event.actoptions.targetId || '';
            event.actoptions.properties = event.actoptions.properties || [];
        }
    }

    onAddProperty(event: GaugeEvent): void {
        if (!event.actoptions.properties) {
            event.actoptions.properties = [];
        }
        event.actoptions.properties.push({ name: 'text', value: '' });
    }

    onRemoveProperty(event: GaugeEvent, index: number): void {
        event.actoptions.properties.splice(index, 1);
    }
