...
import { applyButtonStyle, HtmlButtonStyle } from './html-button-presets';
...
export class HtmlButtonComponent extends GaugeBaseComponent {
    ...
    static actionsType = { hide: GaugeActionsType.hide, show: GaugeActionsType.show, blink: GaugeActionsType.blink, enable: GaugeActionsType.enable, disable: GaugeActionsType.disable };
    ...
    static initElement(gab: GaugeSettings, textTranslation?: string): HTMLElement {
        ...
        if (ele && gab.property) {
            ...
            if (htmlButton) {
                ...
                // 应用自定义样式（边框 / 圆角 / 内边距 / customStyle，不含预设主题）
                const style: HtmlButtonStyle = gab.property.options?.style;
                if (style) {
                    applyButtonStyle(htmlButton, style);
                }
            }
        }
        return ele;
