...
export class ChartUplotComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    private updateCanvasOptions(ngup: NgxUplotComponent) {
        ...
        // Apply axisConfig: hide/show Y-axes based on per-line settings
        if (this.options.axisConfig) {
            for (const [key, cfg] of Object.entries(this.options.axisConfig)) {
                const axisIdx = Number(key);  // axisConfig key 1~4 maps to axes[1]~axes[4]
                if (axisIdx >= 1 && axisIdx < this.options.axes.length) {
                    if (cfg.show === false) {
                        this.options.axes[axisIdx].show = false;
                        this.options.axes[axisIdx].size = 0;
                    } else {
                        this.options.axes[axisIdx].show = true;
                        delete this.options.axes[axisIdx].size;
                    }
                }
            }
        }
