        ...
        if (pro.type === TableType.data && pro.options?.rows) {
            ...
            pro.options.rows.forEach(row => {
                row.cells.forEach(cell => {
                    if (cell && cell.variableId && (cell.type === TableCellType.variable || cell.type === TableCellType.input || cell.type === TableCellType.switch || cell.type === TableCellType.select)) {
                        signalIds.push(cell.variableId);
                    }
                    // Include action-bound variable IDs so they receive real-time updates
                    if (cell?.actions) {
                        cell.actions.forEach(action => {
                            if (action.variableId && signalIds.indexOf(action.variableId) < 0) {
                                signalIds.push(action.variableId);
                            }
                        });
                    }
                });
