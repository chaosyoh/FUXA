        ...
        if (ele) {
            ...
            if (svgIframeContainer) {
                ...
                if (gaugeSettings.property && gaugeSettings.property.address && isview) {
                    let url = gaugeSettings.property.address;
                    // 检查地址是否以 http 或 https 开头
                    if (!location.origin.startsWith('http://localhost') && !url.startsWith('http://') && !url.startsWith('https://')) {
                        // 假设项目前端地址是一个常量，这里用示例值代替，你需要根据实际情况修改
