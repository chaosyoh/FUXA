...
import { HtmlImageListComponent } from './controls/html-imagelist/html-imagelist.component';
...
import { HtmlAlarmListComponent } from './controls/html-alarm-list/html-alarm-list.component';
import { AlarmListGaugeComponent } from './controls/html-alarm-list/alarm-list/alarm-list.component';
...
export class GaugesManager {
    ...
    static GaugeWithActions = [ApeShapesComponent, PipeComponent, ProcEngComponent, ShapesComponent, HtmlButtonComponent, HtmlSelectComponent,
        ValueComponent, HtmlInputComponent, GaugeSemaphoreComponent, HtmlImageComponent, HtmlImageListComponent, PanelComponent, HtmlVideoComponent, GaugeProgressComponent];
    ...
    static Gauges = [ValueComponent, HtmlInputComponent, HtmlButtonComponent, HtmlBagComponent,
        ...
        HtmlImageListComponent, HtmlImageComponent, PanelComponent, HtmlVideoComponent, HtmlSchedulerComponent, HtmlAlarmListComponent];
    ...
    createGaugeStatus(ga: GaugeSettings): GaugeStatus {
        ...
        if (!ga.type.startsWith(HtmlChartComponent.TypeTag) && !ga.type.startsWith(HtmlGraphComponent.TypeTag) &&
            ...
            !ga.type.startsWith(HtmlAlarmListComponent.TypeTag)) {
    ...
    initInEditor(ga: GaugeSettings, res: any, ref: any, elementWithLanguageText?: any) {
        if (ga.type.startsWith(GaugeProgressComponent.TypeTag)) {
            ...
            this.mapGauges[ga.id] = gauge;
        } else if (ga.type.startsWith(HtmlAlarmListComponent.TypeTag)) {
            delete this.mapGauges[ga.id];
            let gauge = HtmlAlarmListComponent.detectChange(ga, res, ref);
            this.mapGauges[ga.id] = gauge;
        } else if (ga.type.startsWith(HtmlImageListComponent.TypeTag)) {
            HtmlImageListComponent.detectChange(ga, true);
    ...
    bindGaugeEventToSignal(ga: GaugeSettings) {
        if (ga.type.startsWith(SliderComponent.TypeTag)) {
            ...
            // no bindEvents needed for imagelist
        } else if (ga.type.startsWith(HtmlImageComponent.TypeTag)) {
            let self = this;
    ...
    static getPrefixGaugeName(type: string) {
        if (type.startsWith(GaugeProgressComponent.TypeTag)) {
            ...
            return 'scheduler_';
        } else if (type.startsWith(HtmlAlarmListComponent.TypeTag)) {
            return 'alarmlist_';
        }
        return 'shape_';
    }
    ...
    initElementAdded(ga: GaugeSettings, res: any, ref: any, isview: boolean, parent?: FuxaViewComponent, textTranslation?: string, sourceDeviceTags?: Tag[]) {
        ...
        if (ga.type.startsWith(HtmlChartComponent.TypeTag)) {
            ...
            if (gauge) {
                gauge.gaugesManager = this;
                // Set up callback to delegate card/dialog events to parent FuxaViewComponent
                if (parent) {
                    const gaId = ga.id;
                    gauge.onViewOpen = (actionType: string, viewId: string, event: any, options: any) => {
                        if (actionType === 'onwindow') {
                            parent.onOpenCard(gaId, event, viewId, options || {});
                        } else if (actionType === 'ondialog') {
                            parent.openDialog(event, viewId, options || {});
                        }
                    };
                }
            ...
            let gauge = HtmlAlarmListComponent.initElement(ga, res, ref, isview);
            if (gauge) {
                this.mapGauges[ga.id] = gauge;
            }
            return gauge;
            ...
            let gauge = HtmlImageListComponent.initElement(ga, isview);
            return gauge || true;
        } else if (ga.type.startsWith(HtmlImageComponent.TypeTag)) {
        ...
        if (property) {
            if (property.id) {
                ...
                if (chart) {
                    ...
                    // Pre-apply Y-axis visibility and labels via axisConfig (axes default to show:false)
                    const axisConfig: { [idx: number]: { show?: boolean; label?: string } } = {};
                    for (let i = 0; i < chart.lines.length; i++) {
                        let line = chart.lines[i];
                        if (line.yaxis >= 1 && line.yaxis <= 10) {
                            if (!axisConfig[line.yaxis]) { axisConfig[line.yaxis] = {}; }
                            // Axes default to hidden; show axis only if yaxisShow is not explicitly false
                            if (line.yaxisShow === false) {
                                axisConfig[line.yaxis].show = false;
                            } else {
                                axisConfig[line.yaxis].show = true;
                            }
                            if (line.yaxisLabel) {
                                axisConfig[line.yaxis].label = line.yaxisLabel;
                            }
                        }
                    }
                    if (Object.keys(axisConfig).length > 0) {
                        opt.axisConfig = axisConfig;
                    }
