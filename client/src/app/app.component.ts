...
export class AppComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	private subscriptionProjectUpdate: Subscription;
	...
	ngOnInit() {
		...
		this.subscriptionProjectUpdate = this.hmiService.onProjectUpdated.subscribe((message: any) => {
			if (this.isEditorOrLabMode()) {
				return;
			}
			...
			console.log('[App] Project update check - local:', localVersion, 'server:', serverTimestamp);
			if (localVersion !== serverTimestamp) {
				this.projectUpdateAvailable = true;
			}
		});
	...
	ngOnDestroy() {
		try {
			...
			if (this.subscriptionProjectUpdate) {
				this.subscriptionProjectUpdate.unsubscribe();
			}
		} catch (e) {
		}
	...
	onDismissUpdate() {
		this.projectUpdateAvailable = false;
	}

	/**
	 * Check if current route is in editor mode or lab mode
	 */
	private isEditorOrLabMode(): boolean {
