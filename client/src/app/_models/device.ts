...
export class Device {
    ...
    /** Tags list of Tag */
    tags: DictionaryTag;
    /** Tag groups for folder/tree organization */
    tagGroups?: DictionaryTagGroup;
    /** Device folder id, empty or undefined means root level */
    folderId?: string;
...
export interface DictionaryTagGroup {
    [id: string]: TagGroup;
}
...
export class Tag {
    ...
    divisor: number;
    /** Read/Write access permission: 'rw' (read-write, default) or 'ro' (read-only) */
    access: string;
    ...
    unsPath?: string | null;
    /** Optional group id, references TagGroup.id */
    groupId?: string;
    /** Deadband to set changed value */
    deadband?: TagDeadband;
    /** Expression for calculated tag type, e.g. '{DeviceA.Tag1} + {DeviceB.Tag2} * 2' */
    expression?: string;
    /**
...
export class DeviceNetProperty {
    ...
    rack: string;
    /** CPU type used for Siemens S7 connection */
    cpuType: string;
...
export const TAG_GROUP_PREFIX = 'tg_';
export const DEVICE_FOLDER_PREFIX = 'df_';

export class TagGroup {
    /** Group id, GUID */
    id: string;
    /** Parent group id, empty string means root level */
    parentId: string;
    /** Device id this group belongs to */
    deviceId: string;
    /** Group display name */
    name: string;

    constructor(_id: string, _deviceId: string, _name: string, _parentId?: string) {
        this.id = _id;
...
export class DeviceFolder {
    ...
    constructor(_id: string, _name: string, _parentId?: string) {
        this.id = _id;
        this.name = _name;
        this.parentId = _parentId || '';
    }
...
export class DevicesUtils {
    ...
    private static formatCsvValue(value: any): string {
        ...
        return text
            ...
            .replace(/\n/g, ' ')
            .replace(/\r/g, ' ')
            .replace(new RegExp(DevicesUtils.columnDelimiter, 'g'), DevicesUtils.columnMaske);
    }
    ...
    private static collectDeviceKeys(devices: Device[]): { deviceKeys: string[]; propertyKeys: string[] } {
        const baseDeviceKeys = Object.keys(Device.descriptor).filter(k => k !== 'tags' && k !== 'property' && k !== 'tagGroups');
        ...
        console.log('baseDeviceKeys', baseDeviceKeys);
        console.log('extraDeviceKeys', extraDeviceKeys);
        devices.forEach(device => {
            ...
            Object.keys(device).forEach(key => {
                ...
                console.log('deviceKeys', `${device.name} - ${key},`);
    ...
    static devicesToCsv(devices: Device[], scripts?: Script[]): string {
        ...
        console.log('deviceKeysInfo', dkeys, pkeys);
        dkeys.forEach(hk => {
            ...
            console.log('devicesHeader 721', hk,devicesHeader);
        });
        ...
        pkeys.forEach(pk => {
            ...
            console.log('devicesHeader 727', devicesHeader);
        });
        // device data
        for (let i = 0; i < devices.length; i++) {
    ...
    static line2TagDynamic(
        ...
        if (!data.id) {
            data.id = Utils.getGUID(TAG_PREFIX);
        }
        const tagId = data.id;
        if (!deviceId) {
            throw new Error(`Tag missing deviceId`);
        }
        let tag = new Tag(tagId);
