...
export class Hmi {
    ...
    views: View[] = [];
    /** View folders for organizing views in tree structure */
    viewFolders: ViewFolder[] = [];
    mobileLayout: LayoutSettings = new LayoutSettings();
}
...
export class View {
    ...
    /** Folder id for organizing views, null/undefined = root level */
    folderId?: string;
...
export enum GaugeActionsType {
    ...
    enable = 'shapes.action-enable',
    disable = 'shapes.action-disable',
}
...
export class GaugeStatus {
    ...
    disabled = false;           // button disabled state (blocks event execution)
    activeHiders?: {[key: string]: boolean};  // tracks which hide actions are currently active
}
...
export enum GaugeEventActionType {
    ...
    onSetControlProp = 'shapes.event-onsetcontrolprop',
}
...
export enum GaugeEventPositionType {
    fixed = 'fixed',
    custom = 'custom'
}

export enum GaugeEventFixedPositionV {
    top = 'top',
    center = 'center',
    bottom = 'bottom'
}

export enum GaugeEventFixedPositionH {
    left = 'left',
    center = 'center',
    right = 'right'
}
...
export interface ControlPropertyItem {
    name: 'text' | 'visibility' | 'backgroundColor' | 'color' | 'enabled' | 'fontSize' | 'border';
    value: any;
}
...
export interface GaugeAlarmListProperty {
    groups: string[];
    columns: string[];
...
export interface AlarmListOptions {
    header: { show: boolean; height: number; background: string; color: string; fontSize: number };
    row: { height: number; fontSize: number; background: string; color: string };
    paginator: { show: boolean; pageSize: number; fontSize: number };
...
export class TableCell {
    ...
    bitmask: number;
    type: TableCellType;
    /** Color-control actions bound to this cell (data-table rows only). */
    actions?: GaugeAction[];
    /** Input cell configuration (when type === input). */
    inputOptions?: TableCellInputOptions;
    /** Button cell configuration (when type === button). */
    buttonOptions?: TableCellButtonOptions;
    /** Button cell click events (when type === button). */
    buttonEvents?: GaugeEvent[];
    /** Input cell enter events (when type === input). */
    inputEvents?: GaugeEvent[];
    /** Switch cell configuration (when type === switch). */
    switchOptions?: TableCellSwitchOptions;
    /** Switch cell events (when type === switch). */
    switchEvents?: GaugeEvent[];
    /** Select cell configuration (when type === select). */
    selectOptions?: TableCellSelectOptions;
    /** Select cell events (when type === select). */
    selectEvents?: GaugeEvent[];
...
/** Input cell configuration options — simplified version of InputOptionsProperty for table cells. */
export interface TableCellInputOptions {
    type?: InputOptionType;
    min?: number;
    max?: number;
    updated?: boolean;
...
/** Button cell configuration options for table cells. */
export interface TableCellButtonOptions {
    text?: string;
    style?: TableCellButtonStyle;
}

/** Simplified button style for table cells. */
export interface TableCellButtonStyle {
    backgroundColor?: string;
    color?: string;
    borderRadius?: string;
    border?: string;
...
/** Switch cell configuration options — matches standalone SwitchOptions for table cells. */
export interface TableCellSwitchOptions {
    offValue?: number;
    onValue?: number;
    ...
    offText?: string;
    onText?: string;
    offSliderColor?: string;
    onSliderColor?: string;
    offTextColor?: string;
    onTextColor?: string;
    fontSize?: number;
    radius?: number;
...
/** Select cell configuration options — matches standalone HtmlSelect for table cells. */
export interface TableCellSelectOptions {
    items?: TableCellSelectItem[];
    readonly?: boolean;
}

/** Single option item for a select cell. */
export interface TableCellSelectItem {
    value: number;
    text?: string;
    color?: string;
    stroke?: string;
