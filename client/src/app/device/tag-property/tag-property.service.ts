...
import { TagPropertyOptionsResult } from './tag-property-options/tag-property-options.component';
...
export class TagPropertyService {
        ...
        return dialogRef.componentInstance.result.pipe(
            map(result => {
                if (result) {
                    ...
                    this.applyTagOptions(tag, result);
                    if (checkToAdd) {
        ...
        return dialogRef.componentInstance.result.pipe(
            map(result => {
                if (result) {
                    ...
                    tag.description = result.tagDescription;
                    tag.expression = result.tagExpression;
                    this.applyTagOptions(tag, result);
                    // For calculated tags: force read-only and set badQualityMode in options
                    if (result.tagType === 'calculated') {
                        tag.access = 'ro';
                        tag.options = { ...(typeof tag.options === 'object' ? tag.options : {}), badQualityMode: result.tagBadQualityMode || 0 };
                    }
                    if (checkToAdd) {
                        this.checkToAdd(tag, device);
                    } else if (tag.id !== oldTagId) {
    ...
    applyTagOptions(tag: Tag, result: any) {
        if (result.daq) {
            tag.daq = result.daq;
        }
        ...
        if (result.tagAccess !== undefined) {
            tag.access = result.tagAccess;
        }
        tag.deadband = result.deadband;
        tag.scale = result.scale;
        tag.scaleReadFunction = result.scaleReadFunction;
        tag.scaleReadParams = result.scaleReadParams;
        tag.scaleWriteFunction = result.scaleWriteFunction;
        tag.scaleWriteParams = result.scaleWriteParams;
    }
