...
import { DeviceType, DeviceSecurity, MessageSecurityMode, SecurityPolicy, ModbusOptionType, ModbusReuseModeType, RedisReadModeType, RedisOptions, S7CpuType, DeviceFolder } from './../../_models/device';
...
export class DevicePropertyComponent implements OnInit, OnDestroy {
	...
	writeArgsTooltip = '';
	result = '';
	s7CpuTypes = Object.values(S7CpuType);
	/** Available device folders for selection */
	deviceFolders: { id: string; name: string }[] = [];
	...
	ngOnInit() {
		...
		// Load device folders for selection
		this.loadDeviceFolders();
		// Ensure folderId is a string for mat-select binding
		if (!this.data.device.folderId) {
			this.data.device.folderId = '';
		}
	...
	/** Load device folders as flat list for the dropdown */
	private loadDeviceFolders() {
		const folders = this.projectService.getDeviceFolders() as { [id: string]: DeviceFolder };
		const list: { id: string; name: string }[] = [];
		// Build a display name with path for nested folders
		const buildPath = (folder: DeviceFolder): string => {
			if (!folder.parentId) return folder.name;
			const parent = folders[folder.parentId];
			return parent ? buildPath(parent) + ' / ' + folder.name : folder.name;
		};
		for (const f of Object.values(folders)) {
			list.push({ id: f.id, name: buildPath(f) });
		}
